	...
	return (
		<div className="p-6 space-y-6">
			...
			<section className="flex items-center gap-3">
				<button className="rounded bg-black px-3 py-2 text-white cursor-pointer" onClick={() => setOpen(true)}>
					Edit Profile
				</button>

				<button className="rounded bg-red-600 px-3 py-2 text-white cursor-pointer" onClick={() => logout(undefined, { onSuccess: () => router.replace('/signin') })} disabled={loggingOut}>
			...
			<Modal open={open} onClose={() => setOpen(false)} title="Edit Profile">
				...
				<EditNameForm onSuccess={() => setOpen(false)} />
			</Modal>
		</div>
	);
}
